...
const init = async () => {
    ...
    try {
        ...
        try { renderDocumentaryAuditSection(); } catch(e) { console.warn("Error en renderDocumentaryAuditSection:", e); }
...
// ── AUDITORÍA DOCUMENTAL Y DETECCIÓN DE OMITIDOS (UI) ───────────────
let currentAuditFindings = [];
let activeModalFindingId = null;
let chartDocAuditConcept = null;

const renderDocumentaryAuditSection = () => {
    if (typeof runDocumentaryAudit !== 'function') {
        console.warn("runDocumentaryAudit no está disponible.");
        return;
    }
    currentAuditFindings = runDocumentaryAudit(rawExpenses);
    ...
    if (cleanEl) {
        const allP = [...new Set(rawExpenses.map(e => e.periodo))];
        ...
        return `
            <tr>
                ...
                <td style="font-weight:500;">
                    ...
                    <div style="font-size:0.7rem; color:var(--text-3);">Monto est. histórico: ${fmt(f.montoEstimado)} <span style="font-size:0.65rem; color:var(--accent);">(ESTIMACIÓN HISTÓRICA)</span></div>
                </td>
...
const openDocAuditModal = (findingId) => {
    ...
    rationaleBox.innerHTML = `
        ...
        <div style="margin-top:0.75rem; padding-top:0.6rem; border-top:1px dashed rgba(6,182,212,0.2); font-size:0.75rem; color:var(--text-3); display:flex; flex-wrap:wrap; gap:12px;">
            ...
            <span>💰 <strong>Promedio Histórico:</strong> ${fmt(finding.montoEstimado)}</span>
        </div>
    `;
